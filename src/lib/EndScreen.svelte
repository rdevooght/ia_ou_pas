<script>
	...
	let share_text = $derived(
		`Challenge: distingue les textes écrits par une IA de ceux écrits par un humain. Mon score: ${correctGuesses}/${results.length}`
	);
	const share_url = 'https://blog.robindevooght.be/ia_ou_pas';
	...
	<div class="flex flex-col gap-4 p-4">
		...
		<div class="flex flex-row justify-center gap-2">
			...
			<a
				...
				class="rounded bg-[#1877f2] px-4 py-2 text-white"
			...
			<!-- LinkedIn -->
			<a
				href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(share_url)}&title=${encodeURIComponent(share_text)}`}
				target="_blank"
				rel="noopener noreferrer"
				class="rounded bg-[#0077b5] px-4 py-2 text-white"
			...
			<a
				...
				target="_blank"
				rel="noopener noreferrer"
				class="rounded bg-[#25D366] px-4 py-2 text-white"
